import axios from "axios";
import HandleExceptions from "./Exceptions/HandleExceptions";

const tokenCookie = "@blueshark-personal-token";
const userCookie = "@blueshark-personal-user";

const isProd = true;
const awsURL = "http://ec2-3-19-249-21.us-east-2.compute.amazonaws.com:8080";
const localURL = "http://localhost:8080";

const baseURL = isProd ? awsURL : localURL;

const timeout = 1000 * 60;
const api = axios.create({
  baseURL,
  timeout,
  timeoutErrorMessage: "Timeout",
});

async function post(url: string, data?: any) {
  const token = JSON.parse(localStorage.getItem(tokenCookie) as string);
  let response: any = {};
  await api
    .post(url, data, {
      headers: {Authorization: "Bearer " + token},
    })
    .then(res => {
      response.data = res.data;
    })
    .catch(error => {
      response.error = HandleExceptions(error);
    });
  return response;
}

async function put(url: string, data?: any) {
  const token = JSON.parse(localStorage.getItem(tokenCookie) as string);
  let response: any = {};
  await api
    .put(url, data, {
      headers: {Authorization: "Bearer " + token},
    })
    .then(res => {
      response.data = res.data;
    })
    .catch(error => {
      response.error = HandleExceptions(error);
    });
  return response;
}

async function patch(url: string, data?: any) {
  const token = JSON.parse(localStorage.getItem(tokenCookie) as string);
  let response: any = {};
  await api
    .patch(url, data, {
      headers: {Authorization: "Bearer " + token},
    })
    .then(res => {
      response.data = res.data;
    })
    .catch(error => {
      response.error = HandleExceptions(error);
    });
  return response;
}

async function get(url: string, params?: any) {
  let response: any = {};
  let token = JSON.parse(localStorage.getItem(tokenCookie) as string);
  await api
    .get(url, {
      params: params,
      headers: {Authorization: "Bearer " + token},
    })
    .then(res => {
      if (params) {
        response.pageNumber = res?.data?.pageable?.pageNumber;
        response.totalPages = res?.data?.totalPages - 1;
      }
      response.data = res.data;
    })
    .catch(error => {
      response.error = HandleExceptions(error);
    });
  return response;
}

async function remove(url: string) {
  let response: any = {};
  let token = JSON.parse(localStorage.getItem(tokenCookie) as string);
  await api
    .delete(url, {
      headers: {Authorization: "Bearer " + token},
    })
    .then(res => {
      response.data = res.data;
    })
    .catch(error => {
      response.error = HandleExceptions(error);
    });
  return response;
}

export {api, baseURL, tokenCookie, userCookie, get, post, put, patch, remove};